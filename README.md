...
4. Make sure you have `librealsense v2.50.0` installed. You can find out your version but running `realsense-viewer`, it should be shown somewhere in the viewer gui.
5. Download `realsense-ros v2.3.2` from this link: https://github.com/IntelRealSense/realsense-ros/releases/tag/2.3.2. Move `realsense-ros-2.3.2` to `~/ros/franka_ws/src/`
7. (only if you have a D405 realsense) Next, you update `franka_ws/src/realsense-ros-2.3.2/src/realsense_node_factory.cpp` to support the D405. Specificially, look for `switch(pid)` on line 357. Simply add `case 0x0B5B:    // D405` into to switch case.
8. Setup a python environment. If you are installing `panda_utils` then you can use the same one created there. Run `pip install "empy==3.3.4" catkin_pkg`
